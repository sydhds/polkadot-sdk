...
fn execute_riscv(program: &[u8]) {
	...
	assert_eq!(host_fn::destroy(instance_id), Ok(()));
	...
	assert_eq!(host_fn::destroy(instance_id), Err(VirtDestroyError::InvalidInstance));
	assert_eq!(ret, Err(ExecError::Trap));
	assert!(state.exit);
	...
	let instance_id_last = host_fn::instantiate(program).unwrap();
	let mut state =
		SharedState { gas_left: 0, exit: false, user: State { counter: 0, instance_id } };
	let ret = host_fn::execute(
		instance_id,
	...
	assert_eq!(host_fn::destroy(instance_last), Err(VirtDestroyError::InvalidInstance));
	assert_eq!(host_fn::destroy(instance_id), Ok(()));
	assert_eq!(ret, Err(ExecError::Trap));
	assert!(!state.exit);
